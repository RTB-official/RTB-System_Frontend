...
const splitEntryByDay = (entry: {
  ...
  if (!entry.dateFrom || !entry.timeFrom || !entry.dateTo || !entry.timeTo) return [];
  ...
  if (entry.descType === '작업' && !entry.noLunch) {
    ...
    const checkLunch = (s: Date, e: Date) => {
      for (let d = new Date(s); d < e; d.setDate(d.getDate() + 1)) {
        const lunchStart = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 12, 0);
        const lunchEnd = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 13, 0);
  ...
  while (cur < end) {
    ...
    const dateKey = `${segStart.getFullYear()}-${String(segStart.getMonth() + 1).padStart(2, '0')}-${String(segStart.getDate()).padStart(2, '0')}`;

    // 분 계산 (하루 내)
    const minutesFromDayStart = (dt: Date) => {
      const base = new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), 0, 0, 0);
      return Math.max(0, Math.min(1440, Math.round((dt.getTime() - base.getTime()) / 60000)));
    };

    const startMin = minutesFromDayStart(segStart);
    const endMin = segStart.getDate() === segEnd.getDate() 
...
const aggregatePersonDayData = (entries: any[]) => {
  const dayPersonMap = new Map<string, { 작업: number; 이동: number; 대기: number }>();
...
const getTypeColor = (type: string) => {
  switch (type) {
    case '작업': return { bg: '#3b82f6', bgDark: '#2563eb', border: '#1d4ed8' };
    case '이동': return { bg: '#10b981', bgDark: '#059669', border: '#047857' };
    case '대기': return { bg: '#f59e0b', bgDark: '#d97706', border: '#b45309' };
    default: return { bg: '#9ca3af', bgDark: '#6b7280', border: '#4b5563' };
  }
};
  ...
  return (
    ...
    >
      ...
      <p className="text-[#6a7282] mb-1">
        {segment.dateKey} {minutesToLabel(segment.startMin)}시 ~ {minutesToLabel(segment.endMin)}시
      </p>
...
export default function TimelineSummarySection({ onDraftSave, onSubmit }: TimelineSummarySectionProps) {
  const { workLogEntries } = useWorkReportStore();
  const [hoveredSegment, setHoveredSegment] = useState<{ segment: DaySegment; position: { x: number; y: number } } | null>(null);
  ...
  if (sortedDates.length === 0) {
    return (
      <div className="bg-white border border-[#e5e7eb] rounded-2xl p-4 md:p-7">
        <h2 className="text-[18px] md:text-[22px] font-semibold text-[#364153] mb-4">타임라인</h2>
        <p className="text-[#99a1af] text-center py-8">저장된 스케줄이 없습니다.</p>
      </div>
    );
  }
  ...
  const renderTimeLabels = (segments: DaySegment[]) => {
    ...
    segments.forEach(seg => {
      ...
      if (!usedPositions.has(startKey)) {
        ...
        labels.push({ min: seg.startMin, left: (seg.startMin / 1440) * 100, isStart: true });
      }
      if (!usedPositions.has(endKey)) {
        usedPositions.add(endKey);
        labels.push({ min: seg.endMin, left: (seg.endMin / 1440) * 100, isStart: false });
      }
    });
          ...
          return (
            <div key={dateKey} className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-[14px] font-semibold ${isWeekend ? 'text-rose-500' : 'text-slate-700'}`}>
                  {dateKey}
                </span>
                <span className={`text-[12px] ${isWeekend ? 'text-rose-400' : 'text-slate-400'}`}>
                  ({weekday})
                </span>
              </div>
      ...
      {allPersons.length > 0 && (
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[16px] md:text-[18px] font-semibold text-[#364153]">일별 시간표</h3>
                ...
                <tr className="bg-gradient-to-r from-slate-100 to-slate-50">
                  ...
                  {allDates.map((date, idx) => {
                    ...
                    return (
                      <th 
                        key={date} 
                        className={`px-3 py-2 text-center border-b border-slate-200 min-w-[140px] ${idx > 0 ? 'border-l-2 border-l-rose-300' : ''}`}
                      >
                        <div className="flex flex-col items-center">
                          <span className={`text-[14px] font-bold ${isWeekend ? 'text-rose-500' : 'text-slate-700'}`}>
                            {d.getMonth() + 1}/{d.getDate()}
                          </span>
                          <span className={`text-[11px] ${isWeekend ? 'text-rose-400' : 'text-slate-400'}`}>
                            ({weekday})
                          </span>
                        </div>
                      </th>
              ...
              <tbody>
                {allPersons.map((person, personIdx) => {
                  return (
                    <tr 
                      key={person} 
                      className={`${personIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50'} hover:bg-blue-50 transition-all`}
                    >
                      <td className={`px-4 py-3 font-semibold text-slate-800 border-b border-slate-100 sticky left-0 ${personIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                        {person}
                      </td>
                      {allDates.map((date, idx) => {
                        const key = `${date}|${person}`;
                        const rec = dayPersonData.get(key) || { 작업: 0, 이동: 0, 대기: 0 };
                        const showWait = hasWaitByDate.get(date);
                        const hasData = rec.이동 > 0 || rec.작업 > 0 || rec.대기 > 0;
                        
                        return (
                          <td 
                            key={date} 
                            className={`px-2 py-2 border-b border-slate-100 ${idx > 0 ? 'border-l-2 border-l-rose-300' : ''}`}
                          >
                            {hasData ? (
                              <div className={`grid ${showWait ? 'grid-cols-3' : 'grid-cols-2'} gap-1`}>
                                <div className="flex flex-col items-center p-1.5 bg-emerald-50 rounded-lg">
                                  <span className="text-[10px] text-emerald-600 font-medium">이동</span>
                                  <span className="text-[14px] font-bold text-emerald-700">{toHourStr(rec.이동)}</span>
                                </div>
                                <div className="flex flex-col items-center p-1.5 bg-blue-50 rounded-lg">
                                  <span className="text-[10px] text-blue-600 font-medium">작업</span>
                                  <span className="text-[14px] font-bold text-blue-700">{toHourStr(rec.작업)}</span>
                                </div>
                                {showWait && (
                                  <div className="flex flex-col items-center p-1.5 bg-amber-50 rounded-lg">
                                    <span className="text-[10px] text-amber-600 font-medium">대기</span>
                                    <span className="text-[14px] font-bold text-amber-700">{toHourStr(rec.대기)}</span>
                                  </div>
                                )}
                              </div>
                            ) : (
                              <div className="text-center text-slate-300 text-[12px] py-2">-</div>
                            )}
                          </td>
                        );
                      })}

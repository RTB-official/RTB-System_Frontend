  ...
  return (
    <div className="flex h-screen bg-[#f4f5f7] overflow-hidden">
      ...
      <div
        ...
        `}
      >
        <Sidebar onClose={() => setSidebarOpen(false)} activeMenu="지출 관리" activeSubMenu="구성원 지출 관리" />
      </div>
      ...
      <div className="flex-1 min-w-0 overflow-hidden lg:ml-[239px]">
        ...
        <div className="lg:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
          ...
          <div className="text-sm font-semibold text-gray-800">구성원 지출 관리</div>
          <div className="w-10" />
        </div>
        ...
        <div className="h-full overflow-y-auto p-3">
          <div className="w-full px-6">
            <ExpenseHeader year="2025년" month="11월" title="구성원 지출 관리" />

            <div className="mb-4">
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-base font-semibold text-gray-700">구성원 선택</span>
                <select 
                  value={selectedMember}
                  onChange={(e) => setSelectedMember(e.target.value)}
                  className="border rounded-md px-4 py-2 text-base bg-white min-w-[150px]"
                >
                  {members.map((member) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 items-stretch">
              <div>
                <MileageCard
                  ...
                  onAdd={(item) => setLeftItems((prev) => [item, ...prev])}
                />
              </div>
              <div>
                <ExpenseFormCard
                  initialDate={preselectedDate || undefined}
                  onAdd={(item) => setRightItems((prev) => [item, ...prev])}
                />
              </div>
            </div>
